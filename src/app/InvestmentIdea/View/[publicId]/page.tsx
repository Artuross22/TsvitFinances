...
const ViewInvestmentIdea = ({ params }: Props) => {
  const [investmentIdea, setInvestmentIdea] =
    useState<ViewInvestmentIdea | null>(null);
      ...
      try {
        ...
        setError(
          err instanceof Error
            ? err.message
            : "Failed to fetch investment idea",
        );
  ...
  const handleDelete = async () => {
    ...
    const confirmDelete = window.confirm(
      `Are you sure you want to delete the investment idea "${investmentIdea.name}"?`,
    );
        ...
        <div className="bg-white rounded-lg shadow hover:shadow-lg transition-shadow p-6">
          <div className="mb-4">
            ...
            <p className="text-sm text-gray-500">
              Created on{" "}
              {new Date(investmentIdea.createdAt).toLocaleDateString()}
            </p>
          </div>
