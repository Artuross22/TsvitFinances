                ...
                <div className="space-y-8">
                    <div className="bg-white rounded-xl shadow-lg overflow-hidden">
                        ...
                        <div className="p-6 grid grid-cols-2 gap-4">
                            ...
                            <UserInfoItem label="Nickname" value={user.nickname} icon={BookUser} />
                            <UserInfoItem 
                                label="Name" 
                        ...
                        <div className="p-6">
                            <div className="grid grid-cols-2 gap-4">
                                {user.groupedBalanceFlows.map((group, index) => (
                                    <div 
                                        key={`${group.balanceType}-${index}`} 
                                        className="bg-gray-50 rounded-lg p-4"
                                    >
                                        <div className="flex justify-between items-center mb-4">
                                            <h4 className={`font-semibold ${group.balanceType === 'Outcome' ? 'text-red-600' : 'text-green-600'}`}>
                                                {group.balanceType || '-'}
                                            </h4>
                                            <span className={`font-medium ${group.balanceType === 'Outcome' ? 'text-red-600' : 'text-green-600'}`}>
                                                ...
                                                {group.balanceFlows.map((flow) => (
                                                    ...
                                                    >
                                                        <div className="flex justify-between items-center">
                                                            <span className={`font-semibold text-sm ${flow.sum > 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                                {flow.sum > 0 ? '+' : ''}{formatCurrency(flow.sum)}
                                                            </span>
