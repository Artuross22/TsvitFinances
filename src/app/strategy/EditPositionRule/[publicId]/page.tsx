...
export const Page = ({ params }: Props) => {
  ...
  const handleSubmit = async () => {
    ...
    const invalidRules = positionRule.positionRules.some(
      (rule) =>
    ...
    if (invalidRules) {
      setError(
        "All rules must have a minimum correction percent greater than 0 and a valid time frame",
      );
      return;
    }
            ...
            {positionRule.positionRules.length === 0 ? (
              <div className="text-center py-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Get started by adding a new position rule.
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                {positionRule.positionRules.map((rule, index) => (
                  <div
                    key={index}
                    className="bg-white rounded-lg border border-gray-200 p-6 relative hover:border-gray-300 transition-colors"
                  >
                    <button
                        ...
                        <div className="relative rounded-md shadow-sm">
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              newRules[index] = {
                                ...rule,
                                minimumCorrectionPercent: parseFloat(
                                  e.target.value,
                                ),
                              };
                              setPositionRule({
                        ...
                        >
                          {Object.entries(TimeFrame)
                            ...
                            .filter(([_, value]) => {
                              return (
                                value === rule.timeFrame ||
                                !positionRule.positionRules.some(
                                  (r) => r.timeFrame === value,
                                )
                              );
                            })
