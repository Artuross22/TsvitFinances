...
export const ListPaperAccounts  = () => {
    const [paperAccounts, setPaperAccounts] = useState<PaperAccount>({ error: null, data: [] });
            ...
            {paperAccounts.error ? (
                ...
                paperAccounts.data.map(acc => (
                    ...
                    >
                        ...
                        <div className="text-gray-500 text-sm mb-1">ID: {acc.AccountId}</div>
                        <div className={`font-semibold ${acc.AccountStatus === 'Active' ? 'text-green-600' : 'text-red-600'}`}>Статус: {acc.AccountStatus}</div>
                        <div className="text-gray-400 text-sm mt-1">Тип: {acc.Type}</div>
                        <div className="text-gray-400 text-sm">Валюта: {acc.Currency}</div>
                    </div>
                ))
            ) : (
