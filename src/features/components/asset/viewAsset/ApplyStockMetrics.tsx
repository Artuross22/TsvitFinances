  ...
  const renderMetricRow = (
    ...
    <tr key={label} className="border-b hover:bg-gray-50 transition-colors">
      ...
      <td className="px-4 py-2 text-right">
        {safeToFixed(currentValue)}
        {percentage ? "%" : ""}
      </td>
      <td className="px-4 py-2 text-right">
        {safeToFixed(recommendedValue)}
        {percentage ? "%" : ""}
      </td>
    </tr>
  );
  ...
  return (
    <div className="bg-gray-100 rounded-lg shadow-md p-4 max-w-4xl mx-auto my-8">
      <div className="mb-4">
        ...
        <p className="text-sm text-gray-600">Date: {metrics.date}</p>
      </div>
        ...
        <table className="w-full border-collapse text-sm">
          ...
          <tbody>
            {renderMetricRow(
              "Debt Ratio",
              metrics.debtRatio,
              metrics.recommendedDebtRatio,
            )}
            {renderMetricRow(
              "P/S Ratio",
              metrics.psRatio,
              metrics.recommendedPSRatio,
            )}
            {renderMetricRow(
              "P/B Ratio",
              metrics.pbRatio,
              metrics.recommendedPBRatio,
            )}
            {renderMetricRow(
              "P/E Ratio",
              metrics.peRatio,
              metrics.recommendedPERatio,
            )}
            ...
            {renderMetricRow(
              ...
              metrics.recommendedFreeCashFlow,
            )}
            {renderMetricRow(
              "Net Income",
              metrics.netIncome,
              metrics.recommendedNetIncome,
            )}
            {renderMetricRow(
              "Net Profit Margin",
              metrics.netProfitMargin,
              metrics.recommendedNetProfitMargin,
              true,
            )}
            {renderMetricRow(
              "Dividend Yield",
              metrics.dividendYield,
              metrics.recommendedDividendYield,
              true,
            )}
            {renderMetricRow(
              "Shares Outstanding",
              metrics.sharesOutstanding,
              metrics.recommendedSharesOutstanding,
            )}
            {renderMetricRow(
              "Debt to Equity Ratio",
              metrics.debtToEquityRatio,
              metrics.recommendedDebtToEquityRatio,
            )}
            {renderMetricRow(
              "Free Cash Flow Per Share",
              metrics.freeCashFlowPerShare,
              metrics.recommendedFreeCashFlowPerShare,
            )}
            {renderMetricRow(
              "Operating Cash Flow Per Share",
              metrics.operatingCashFlowPerShare,
              metrics.recommendedOperatingCashFlowPerShare,
            )}
          </tbody>
        </table>
      </div>
